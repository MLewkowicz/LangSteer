...
class DiffuserActor(nn.Module):
    ...
    def conditional_sample(self, condition_data, condition_mask, fixed_inputs,
        ...
        for t in timesteps:
            ...
            # Scheduler steps (position + rotation separately)
            ...
            if corrector_steps > 0 and t > 0:
                alpha_bar_pos = self.position_noise_scheduler.alphas_cumprod[t]
                alpha_bar_rot = self.rotation_noise_scheduler.alphas_cumprod[t]

                for _k in range(corrector_steps):
                    noise_pred = self.policy_forward_pass(
                        trajectory, t_batch, fixed_inputs
                    )[-1]

                    if guidance_fn is not None:
                        guidance = guidance_fn(trajectory, t, fixed_inputs, noise_pred)
                        noise_pred = noise_pred + guidance

                    step_noise = torch.randn_like(trajectory)

                    # Position corrector: score = -eps / sqrt(1 - alpha_bar)
                    score_pos = -noise_pred[..., :3] / torch.sqrt(1 - alpha_bar_pos).clamp(min=1e-6)
                    trajectory[..., :3] = (
                        trajectory[..., :3]
                        + corrector_step_size * score_pos
                        + (2 * corrector_step_size) ** 0.5 * step_noise[..., :3]
                    )

                    # Rotation corrector
                    score_rot = -noise_pred[..., 3:9] / torch.sqrt(1 - alpha_bar_rot).clamp(min=1e-6)
                    trajectory[..., 3:9] = (
                        trajectory[..., 3:9]
                        + corrector_step_size * score_rot
                        + (2 * corrector_step_size) ** 0.5 * step_noise[..., 3:9]
                    )
    ...
    def forward(
        ...
        Arguments:
            ...
            instruction: (B, max_instruction_length, 512) or None when use_instruction=False
            mask_language: Zero out instruction embeddings for unconditioned inference
            guidance_fn: Optional guidance callback for steering (DPS-style)
            corrector_steps: Number of Langevin MCMC corrector steps per denoising step
