...
class Encoder(nn.Module):
    ...
    def __init__(self,
        ...
        if use_instruction:
            ...
            self.vl_attention = nn.ModuleList([layer])
    ...
    @staticmethod
    def _farthest_point_sampler(points, nsamples, start_idx=0):
        """Pure PyTorch farthest point sampling (replaces dgl.geometry.farthest_point_sampler).

        Args:
            points: (B, N, C) tensor
            nsamples: number of points to sample
            start_idx: index of the first sampled point

        Returns:
            (B, nsamples) tensor of sampled indices
        """
        B, N, _ = points.shape
        device = points.device
        sampled = torch.zeros(B, nsamples, dtype=torch.long, device=device)
    ...
    def run_fps(self, context_features, context_pos):
        ...
        sampled_inds = self._farthest_point_sampler(
            einops.rearrange(
                context_features,
                "npts b c -> b npts c"
            ).to(torch.float64),
    ...
    def vision_language_attention(self, feats, instr_feats):
        assert self.use_instruction, "vision_language_attention called but use_instruction=False"
