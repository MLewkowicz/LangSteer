...
class MultiheadCustomAttention(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None,
        ...
        self.gate_attn = None
        if gate_attn:
            self.gate_attn = Parameter(torch.randn(num_heads))
...
def multi_head_attention_forward(query, key, value, embed_dim_to_check, num_heads,
                                 ...
                                 slot_competition=False, rotary_pe=None,
                                 return_kv=False, k_mem=None, v_mem=None,
                                 gate_attn=None, mem_mask=None):
    ...
    if (gate_attn is not None) and (k_mem is not None) and (v_mem is not None):
        ...
        attn_output_weights_mem = torch.bmm(q, k_mem.transpose(1, 2))
        attn_output_weights_mem = F.softmax(attn_output_weights_mem, dim=-1)
        if mem_mask is not None:
            mem_mask = mem_mask.unsqueeze(1).unsqueeze(1)
        ...
        attn_output_mem = torch.bmm(attn_output_weights_mem, v_mem)

        gate = torch.sigmoid(gate_attn).reshape(-1, 1, 1, 1)
        attn_output_mem = attn_output_mem.view(bsz, num_heads, tgt_len, head_dim).transpose(0, 1)
        attn_output = attn_output.view(bsz, num_heads, tgt_len, head_dim).transpose(0, 1)
