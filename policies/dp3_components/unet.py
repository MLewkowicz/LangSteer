"""Conditional 1D U-Net for diffusion denoising.
...
from policies.dp3_components.conv_components import (
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, cond):
        """
        Args:
            x: (B, T, in_dim) input features
            cond: (B, T_cond, cond_dim) conditioning features

        Returns:
            (B, T, out_dim) attention output
        """
        # Project x and cond to query, key, and value
        query = self.query_proj(x)  # (B, T, out_dim)
        key = self.key_proj(cond)   # (B, T_cond, out_dim)
        value = self.value_proj(cond)  # (B, T_cond, out_dim)

        # Compute attention
        attn_weights = torch.matmul(query, key.transpose(-2, -1))  # (B, T, T_cond)
        attn_weights = F.softmax(attn_weights, dim=-1)

        # Apply attention
        attn_output = torch.matmul(attn_weights, value)  # (B, T, out_dim)
...
class ConditionalResidualBlock1D(nn.Module):
    """
    Conditional residual block for 1D U-Net.
    Supports multiple conditioning types: FiLM, add, cross-attention.
    """
    ...
    def __init__(self,
        ...
        Args:
            ...
            condition_type: Type of conditioning ('film', 'add', 'cross_attention_add', etc.)
        """
        super().__init__()
    ...
    def forward(self, x, cond=None):
        """
        Args:
            x: (B, in_channels, T) input tensor
            cond: (B, cond_dim) or (B, T_cond, cond_dim) conditioning tensor
        ...
        if cond is not None:
            if self.condition_type == 'film':
                ...
                embed = embed.permute(0, 2, 1)  # (B, out_channels, T)
                out = out + embed
...
class ConditionalUnet1D(nn.Module):
    """
    Conditional 1D U-Net for diffusion denoising.
    Supports global and local conditioning with various conditioning types.
    """
    ...
    def __init__(self,
        ...
        """
        Args:
            input_dim: Input/output action dimension
            local_cond_dim: Local conditioning dimension (optional)
            global_cond_dim: Global conditioning dimension (optional)
            diffusion_step_embed_dim: Timestep embedding dimension
            down_dims: Dimensions for downsampling path
            kernel_size: Convolution kernel size
            n_groups: Number of groups for GroupNorm
            condition_type: Type of conditioning ('film', 'add', 'cross_attention', etc.)
    ...
    def forward(self,
        ...
        Args:
            sample: (B, H, T) noisy input tensor
            timestep: (B,) or int, diffusion timestep
            local_cond: (B, H, local_cond_dim) local conditioning (optional)
            global_cond: (B, global_cond_dim) global conditioning (optional)

        Returns:
            (B, H, T) denoised output tensor
        """
