#!/usr/bin/env python3
"""
Submit LangSteer Isaac mug training to Amazon SageMaker.

Run from your laptop with AWS credentials configured:
    python training/sagemaker/submit_isaac_mug_training.py

BEFORE RUNNING
--------------
1. pip install -r training/sagemaker/submit_requirements.txt
2. aws configure  (or use instance/Studio role)
3. Verify data is on S3:
       aws s3 ls s3://calvin-abcd-dataset-bucket/isaac_sim_demos_mug/training/ | wc -l
       aws s3 ls s3://calvin-abcd-dataset-bucket/isaac_sim_demos_mug/validation/ | wc -l
4. export WANDB_API_KEY=<your-key>
5. python training/sagemaker/submit_isaac_mug_training.py

ENV OVERRIDES
-------------
LANGSTEER_BUCKET           S3 bucket name
LANGSTEER_DATA_PREFIX      S3 prefix for the dataset  (default: isaac_sim_demos_mug)
LANGSTEER_INSTANCE_TYPE    SageMaker instance          (default: ml.g6e.8xlarge)
...
DATA_PREFIX    = os.environ.get("LANGSTEER_DATA_PREFIX", "isaac_sim_demos_mug")
SAGEMAKER_ROLE = "arn:aws:iam::317694661330:role/SageMakerExecutionRole"
MODEL_PREFIX   = "langsteer/models/diffuser_actor_isaac_mug"
...
def main() -> None:
    ...
    source_code = SourceCode(
        ...
        entry_script="training/sagemaker/train_isaac_mug_sagemaker.py",
    ...
    trainer_kw: dict = {
        ...
        "base_job_name":      f"langsteer-isaac-mug-{timestamp}",
    ...
    trainer.train(input_data_config=[
        InputData(channel_name="isaac_mug", data_source=data_uri)
    ])
