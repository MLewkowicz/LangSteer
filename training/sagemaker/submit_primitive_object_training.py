#!/usr/bin/env python3
"""
Submit LangSteer 3D Diffuser Actor (primitive + object conditioning) training to SageMaker.
...
3. Data is already on S3 at:
       s3://calvin-abcd-dataset-bucket/calvin_object_action/packaged_ABC_D/
   Verify it's there:
       aws s3 ls s3://calvin-abcd-dataset-bucket/calvin_object_action/packaged_ABC_D/training/
4. Update SAGEMAKER_ROLE below (or set LANGSTEER_SAGEMAKER_ROLE env var).
5. export WANDB_API_KEY=<your-key>
6. python training/sagemaker/submit_primitive_object_training.py

ENV OVERRIDES
-------------
LANGSTEER_BUCKET                  S3 bucket name
LANGSTEER_PRIMITIVE_OBJ_PREFIX    S3 prefix for the primitive+object dataset
LANGSTEER_SAGEMAKER_ROLE          IAM execution role ARN
...
PRIMITIVE_OBJ_DATA_PREFIX = os.environ.get("LANGSTEER_PRIMITIVE_OBJ_PREFIX", "calvin_object_action")
SAGEMAKER_ROLE            = os.environ.get(
    "LANGSTEER_SAGEMAKER_ROLE",
    "arn:aws:iam::317694661330:role/SageMakerExecutionRole",
)
MODEL_PREFIX   = "langsteer/models/diffuser_actor_primitive_object"
...
MAX_RUNTIME    = int(os.environ.get("LANGSTEER_MAX_RUNTIME", 259200))
...
# =============================================================================
# Hyperparameters — forwarded to train_primitive_object_sagemaker.py as strings
# =============================================================================
...
HYPERPARAMETERS = {
    ...
    "num_objects":    "8",   # block=0..slider_handle=7
...
def main() -> None:
    ...
    data_uri  = f"s3://{BUCKET_NAME}/{PRIMITIVE_OBJ_DATA_PREFIX}".rstrip("/") + "/"
    ...
    print(f"Data (S3):        {data_uri}")
    print(f"Output (S3):      {s3_output}/")
    print(f"Instance:         {INSTANCE_TYPE}")
    print(f"Train iters:      {HYPERPARAMETERS['train_iters']}")
    print(f"num_primitives:   {HYPERPARAMETERS['num_primitives']}")
    print(f"num_objects:      {HYPERPARAMETERS['num_objects']}")
    ...
    source_code = SourceCode(
        ...
        entry_script="training/sagemaker/train_primitive_object_sagemaker.py",
    ...
    trainer_kw: dict = {
        ...
        "base_job_name":      f"langsteer-prim-obj-{timestamp}",
    ...
    trainer.train(input_data_config=[
        InputData(channel_name="primitive_object", data_source=data_uri),
    ])
    ...
    print(f"Artifacts (S3):   {s3_output}/")
