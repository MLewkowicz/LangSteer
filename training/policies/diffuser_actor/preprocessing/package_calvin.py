...
def make_env(dataset_path, split):
    ...
    wrapper = CalvinGymWrapper(dataset_path=dataset_path, split=split, use_gui=False)
    env, wrapper._env = wrapper._env, None  # detach so wrapper.__del__ doesn't close it
    return env
